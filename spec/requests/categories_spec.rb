...
RSpec.describe 'Categories API', type: :request do
  ...
  describe 'POST /categories' do
    # valid payload
    let(:valid_attributes) { { title: 'Learn Test', created_by: '1' } }

    context 'when the request is valid' do
      before { post '/categories', params: valid_attributes }

      it 'creates a todo' do
        expect(json['title']).to eq('Learn Test')
      end
    ...
    context 'when the request is invalid' do
      before { post '/categories', params: { title: 'Test' } }
  ...
  describe 'PUT /categories/:id' do
    let(:valid_attributes) { { title: 'Tests' } }
